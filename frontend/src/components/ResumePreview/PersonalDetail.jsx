...
const PersonalDetail = ({ resumeInfo }) => {
    ...
    return (
        <div>
            ...
            <h2 className=' text-center font-normal'>{resumeInfo?.address}</h2>
            {/* <h2 className=' text-center text-sm font-medium'>{resumeInfo?.jobTitle}</h2> */}
            <div className=' flex justify-around'>
                <h2 className=' flex items-center gap-1 font-semibold text-xs underline'><Mail size="12" />{resumeInfo?.email}</h2>
                <h2 className=' flex items-center gap-1 font-semibold text-xs underline'><PhoneIcon size="12" />+91-{resumeInfo?.phone}</h2>
